package com.visium.backend.repository;

import com.visium.backend.entity.PasswordRecoveryCode;
import java.util.List;
import java.util.Optional;
import java.util.UUID;
import org.springframework.data.jpa.repository.JpaRepository;

public interface PasswordRecoveryCodeRepository extends JpaRepository<PasswordRecoveryCode, UUID> {
  List<PasswordRecoveryCode> findByEmailIgnoreCaseAndUsedAtIsNull(String email);

  Optional<PasswordRecoveryCode> findFirstByEmailIgnoreCaseOrderByCreatedAtDesc(String email);
}
